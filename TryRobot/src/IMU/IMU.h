...
constexpr int SAMPLE_TIME = 10;
// Minimum Time interval between each Reading
...
void initializeBNO(){
    ...
    debug_msg(F("MPU6050 status: "));
    debug_msg(String(status));
    debug_msg(F("Calculating offsets, do not move MPU6050"));
    delay(1000);
    mpu.calcOffsets();
    debug_msg("Done!\n");
}
...
float getYaw(){
    ...
    if((currentTime - lastRead)>SAMPLE_TIME){
        mpu.update();
        debug_msg("X : ");
    debug_msg(String(mpu.getAngleX()));
    debug_msg("Y : ");
    debug_msg(String(mpu.getAngleY()));
    debug_msg("Z : ");
    debug_msg(String(mpu.getAngleZ()));
